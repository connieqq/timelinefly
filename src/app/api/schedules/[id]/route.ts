...
import { deleteSchedule, updateSchedule } from "@/lib/file-db";
...
export async function PATCH(
  ...
  const item = await updateSchedule(auth.user.id, context.params.id, {
    date: body.date!,
...
export async function DELETE(
  ...
  const removed = await deleteSchedule(auth.user.id, context.params.id);
  if (!removed) {
    return NextResponse.json({ error: "not_found" }, { status: 404 });
  }
